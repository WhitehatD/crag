...
/**
 * Normalize a rendered .gen file for the "did the substance change?"
 * comparison: strip the volatile as-of banner and collapse the per-rule
 * `adopted:YYYY-MM-DD` stamp. Without this, EVERY distill run would emit a
 * new timestamp -> a spurious git diff -> churn in the propose-don't-impose
 * review flow. We only want a real diff when the underlying principle set
 * (id / confidence / scope / text) actually changed.
 */
function substantiveSignature(content) {
  if (content == null) return null;
  return content
    .replace(/<!--\s*GENERATED by[\s\S]*?-->\s*/g, '')
    .replace(/adopted:\d{4}-\d{2}-\d{2}/g, 'adopted:DATE')
    .trim();
}
...
async function runDistill(opts = {}) {
  ...
  const layers = plan.map(({ layer, path: genPath, principles }) => {
    ...
    // Compare substance only (ignore banner timestamp + adoption dates) so a
    // re-run on an unchanged principle set is a true no-op (no git churn).
    const changed = substantiveSignature(existing) !== substantiveSignature(rendered);
    let wrote = false;
    if (!check && changed) {
